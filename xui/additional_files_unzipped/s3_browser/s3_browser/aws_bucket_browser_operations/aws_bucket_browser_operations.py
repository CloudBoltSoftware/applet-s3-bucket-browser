...
def get_folder_with_items(aws, bucket_name, main_folder, bucket_location, flat=True):
    ...
    try:
        ...
        result_files = (
            get_files(bucket_name, main_folder, result, flat, bucket_location, s3_client)
...
def get_files(bucket_name, main_folder, result, flat, bucket_location, s3client=None, sort_a_z=True ):
    try:
        ...
        for obj in versions:
            ...
            params = {'Bucket': bucket_name, 'Key': key, 'VersionId': obj.get('VersionId')}
            download_url= s3client.generate_presigned_url('get_object', Params=params) if obj.get('Size') is not None else None
            version_dict = {
        ...
        for key, versions in versions_by_key.items():
            ...
            latest_version = next((v for v in versions if v['is_latest']), versions[0])  # get the latest version if available, otherwise use the first version
            item_type = get_file_type(file_name)  # Use the type of the latest version, or extract from the file name if not available
            file_dict = {
                "key": key,
                "url": key,
                "name": file_name,
                ...
                "download_url": latest_version.get('download_url'),  # Use the download_url of the latest version
...
def get_folders(bucket_name, main_folder, delimiter, s3client, sort_a_z):
    ...
    response_with_delimiter = s3client.list_object_versions(Bucket=bucket_name, Prefix=main_folder, Delimiter=delimiter)
    common_prefixes = response_with_delimiter.get("CommonPrefixes", [])
    # Get all delete markers without using the delimiter
    response_without_delimiter = s3client.list_object_versions(Bucket=bucket_name, Prefix=main_folder)
    ...
    try:
        ...
        for obj in common_prefixes:
            ...
            is_folder_deleted = all(
                version['Key'] in deleted_objects for version in response_without_delimiter.get('Versions', []) 
...
def is_versioning_enabled(s3_client, resource_id, default_region):
    ...
    try:
        ...
        region = region_attribute.str_value if region_attribute and region_attribute.str_value else default_region # default_region set from bucket_location in IWH 'S3 Browser'
...
def create_presigned_url(bucket_name, object_name, aws, expiration=3600, version_id=None):
