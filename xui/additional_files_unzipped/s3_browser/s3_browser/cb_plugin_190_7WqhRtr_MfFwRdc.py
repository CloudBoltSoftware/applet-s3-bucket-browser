...
def inbound_web_hook_post(*args, parameters={}, **kwargs):
    ...
    for dir_item in s3_list:
        ...
        if dir_item["is_file"] or not flat:
            ...
            if "versions" in dir_item:
                dir_item["versions"] = sorted(dir_item["versions"], key=lambda v: v['last_modified'], reverse=True)
                ...
                for key, value in latest_version.items():
                    ...
                    if key == 'download_url' and latest_version.get('type') == 'DeleteMarker':
                        continue 
                    dir_item[key] = value
    ...
    logger.info("User %s browsed %s in S3 bucket: %s" % (user, folder_path, resource.name))
    ...
    state.update(
        {
            ...
            "flat": flat,
            "versioning_enabled": operations.is_versioning_enabled(s3_client,resource_id,default_region=bucket_location),
        }
    )
    return {
        "state": state,
