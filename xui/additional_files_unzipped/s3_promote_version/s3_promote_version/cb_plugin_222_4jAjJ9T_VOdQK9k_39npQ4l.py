...
def inbound_web_hook_post(*args, parameters={}, **kwargs):
    ...
    try:
        ...
        logger.info(
            "User %s promoted version %s in S3 bucket: %s"
            % (user, version_id, resource.name)
        )
        ...
        if e.response["Error"]["Code"] == "NoSuchVersion":
            logger.error(
                f"The specified version: {version_id} does not exist in S3 bucket: {resource.name}"
            )
        else:
        ...
        logger.exception(
            "User %s failed to promote version %s in S3 bucket: %s"
            % (user, version_id, resource.name)
        )
        raise Exception
